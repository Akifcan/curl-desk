...
export function CollectionItem({ col, activeRequestId, onSelectRequest, onDeleteCollection, onRenameCollection, onRenameRequest, onDeleteRequest, onSaveToCollection, onDeleteExample, onLoadExample, }) {
    ...
    const [expandedExamples, setExpandedExamples] = useState(null);
    const handleSave = () => {
        if (saveName.trim()) {
                            ...
                            : _jsx(ChevronRight, { size: 12, strokeWidth: 2 }) }), editing ? (_jsx("span", { className: "collection-edit", onClick: (e) => e.stopPropagation(), children: _jsx("input", { autoFocus: true, className: "collection-edit-input", type: "text", value: editName, onChange: (e) => setEditName(e.target.value), onKeyDown: (e) => {
                                ...
                                if (e.key === 'Escape')
                                    setEditing(false);
                                            ...
                                            if (e.key === 'Escape')
                                                setEditingReqId(null);
                                        }, onBlur: handleRenameRequest })) : (_jsx("span", { className: "request-name", onDoubleClick: (e) => { e.stopPropagation(); setEditingReqId(req.id); setEditReqName(req.name || ''); }, children: req.name || req.url || 'Untitled' })), editingReqId !== req.id && (_jsxs("span", { className: "request-actions", children: [(req.examples?.length ?? 0) > 0 && (_jsxs("button", { className: "icon-btn", onClick: (e) => { e.stopPropagation(); setExpandedExamples(expandedExamples === req.id ? null : req.id); }, title: "Examples", children: [_jsx(FileText, { size: 10, strokeWidth: 2.5 }), _jsx("span", { className: "example-count", children: req.examples.length })] })), _jsx("button", { className: "icon-btn", onClick: (e) => { e.stopPropagation(); setEditingReqId(req.id); setEditReqName(req.name || ''); }, title: "Rename request", children: _jsx(Pencil, { size: 10, strokeWidth: 2.5 }) }), _jsx("button", { className: "icon-btn delete-btn", onClick: (e) => { e.stopPropagation(); onDeleteRequest(col.id, req.id); }, title: "Delete request", children: _jsx(X, { size: 11, strokeWidth: 2.5 }) })] }))] }), expandedExamples === req.id && req.examples && req.examples.length > 0 && (_jsx("div", { className: "examples-list", children: req.examples.map((ex) => (_jsxs("div", { className: "example-item", onClick: () => onLoadExample(ex.response), children: [_jsx(FileText, { size: 10, strokeWidth: 2, className: "example-icon" }), _jsx("span", { className: "example-name", children: ex.name }), _jsx("span", { className: `example-status status-${Math.floor(ex.response.status / 100)}xx`, children: ex.response.status }), _jsx("button", { className: "icon-btn delete-btn", onClick: (e) => { e.stopPropagation(); onDeleteExample(col.id, req.id, ex.id); }, title: "Delete example", children: _jsx(X, { size: 10, strokeWidth: 2.5 }) })] }, ex.id))) }))] }, req.id))), saving ? (_jsxs("div", { className: "sidebar-input-row save-row", children: [_jsx("input", { autoFocus: true, className: "sidebar-input", type: "text", placeholder: "Request name...", value: saveName, onChange: (e) => setSaveName(e.target.value), onKeyDown: (e) => {
                                    if (e.key === 'Enter')
                                        handleSave();
                                    if (e.key === 'Escape') {
                                        setSaving(false);
